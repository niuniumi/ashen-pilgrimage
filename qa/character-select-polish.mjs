...
async function startCharacterSelect(page) {
  ...
  await page.waitForFunction(() => {
    ...
    const active = game?.scene?.getScenes(true).map((item) => item.scene.key) ?? [];
    return active.length === 1
      && active[0] === 'CharacterSelectScene'
...
try {
  captures = [
    await capture({ width: 1536, height: 864 }, 'character-select-default'),
