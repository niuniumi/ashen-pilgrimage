...
export function impactBurst(scene, x, y, amount = 0, color = 0xf3d48b) {
  ...
  const radius = 20 + strength * 0.45;
  burst.fillStyle(color, 0.9);
  burst.fillRect(x - radius, y - 4, radius * 2, 8);
  burst.fillRect(x - 4, y - radius, 8, radius * 2);
  ...
  for (let i = 0; i < 10; i += 1) {
    ...
    const px = Math.round((x + Math.cos(angle) * outer) / 4) * 4;
    const py = Math.round((y + Math.sin(angle) * outer) / 4) * 4;
    burst.fillRect(px - 4, py - 4, 8, 8);
  }
  scene.tweens.add({
    targets: burst,
    alpha: 0,
...
export function bossPhaseSurge(scene, target, phase = 2) {
  ...
  const surge = phase >= 3 ? 0xffd080 : 0xe46b4b;
  ring.fillStyle(surge, 0.86);
  ring.fillRect(x - 80, y - 80, 160, 8);
  ring.fillRect(x - 80, y + 72, 160, 8);
  ring.fillRect(x - 80, y - 72, 8, 144);
  ring.fillRect(x + 72, y - 72, 8, 144);
  ring.fillStyle(0xffffff, 0.58);
  ring.fillRect(x - 100, y - 100, 200, 4);
  ring.fillRect(x - 100, y + 96, 200, 4);
  ring.setScale(0.55);
